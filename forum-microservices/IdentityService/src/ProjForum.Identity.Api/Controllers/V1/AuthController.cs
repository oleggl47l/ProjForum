...
public class AuthController(IMediator mediator) : ControllerBase
{
    [HttpPost("login")]
    [AllowAnonymous]
    [SwaggerOperation(OperationId = "LoginUser", Summary = "Login user", Description = "Logs in a user and returns JWT and refresh token.")]
    ...
    [HttpPost("register")]
    [AllowAnonymous]
    [SwaggerOperation(OperationId = "RegisterUser", Summary = "Register user", Description = "Registers a new user with specified roles.")]
    public async Task<ActionResult<OperationResultDto>> Register([FromBody] RegisterUserCommand command, CancellationToken cancellationToken)
    {
        var result = await mediator.Send(command, cancellationToken);
        return Ok(result);
    }
    ...
    [HttpPost("logout")]
    [Authorize]
    [SwaggerOperation(OperationId = "LogoutUser", Summary = "Logout user", Description = "Logs out the currently authenticated user.")]
